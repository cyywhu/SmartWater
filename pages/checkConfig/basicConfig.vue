<template>
	<view v-if="showPage">
		<selfTitle title_name="查看基本参数" :selfUrl="returnUrl"></selfTitle>
		...
		<view class="form_body1">
			<view v-for="(i, index) in mainChannels">
				...
				<view v-if="i.isShow" class="form_body2">
					...
					<view v-if="i.type == 0">
						<ipInput input_name="主信道IP地址" :isEdit="isEdit" :initialValue="i.address" @getInputValue="bindNowMainChannelAddress($event, i)"></ipInput>
			...
			<view v-if="isEdit" class="button_block">
				<view v-if="isShowMain" class="form_body2">
					...
					<view v-if="newMainChannel.type == 0">
						<ipInput :isEdit="isEdit" input_name="主信道IP地址" @getInputValue="bindMainChannelAddress"></ipInput>
						<formInput :isEdit="isEdit" :isNessary="true" input_name="主信道端口" @getInputValue="bindMainChannelPort"></formInput>
					</view>
					<view v-if="newMainChannel.type == 1">
						<formInput :isEdit="isEdit" :isNessary="true" input_name="北斗卡号" @getInputValue="bindBeidouCode($event, newMainChannel)"></formInput>
					</view>
					<view class="form_picker_block">
						<view class="form_picker_style">
							<view v-if="true" class="form_picker_plus">*</view>
		...
		<view class="form_body1">
			<view v-for="(i, index) in channels">
				...
				<view v-if="i.isShow" class="form_body2">
					...
					<view v-if="i.type == 0">
						<ipInput :isEdit="isEdit" input_name="备用信道IP地址" :initialValue="i.address" @getInputValue="bindNowChannelAddress($event, i)"></ipInput>
			...
			<view v-if="isEdit" class="button_block">
				<view v-if="isShow" class="form_body2">
					...
					<view v-if="newChannel.type == 0">
						<ipInput :isEdit="isEdit" input_name="备用信道IP地址" @getInputValue="bindChannelAddress"></ipInput>
						<!-- <formInput :isEdit="true" :isNessary="true" input_name="备用信道IP地址" @getInputValue="bindChannelAddress"></formInput> -->
						<formInput :isEdit="isEdit" :isNessary="true" input_name="备用信道端口" @getInputValue="bindChannelPort"></formInput>
					</view>
					<view v-if="newChannel.type == 1">
						<formInput :isEdit="isEdit" :isNessary="true" input_name="北斗卡号" @getInputValue="bindBeidouCode($event, newChannel)"></formInput>
					</view>
					<view class="form_picker_block">
						<view class="form_picker_style">
							<view v-if="true" class="form_picker_plus">*</view>
...
<script>
	...
	export default {
		...
		created() {
			utils.importBasicConfig(this, uni, this.project.name);
		},
		methods: {
			hideSuper(){
				this.isSuper = false;
						...
						success: function (res) {
							if (res.confirm) {
								...
								for(var j = index; j < _this.mainChannels.length; j++){
									...
									console.log(_this.mainChannels[index].address)
								}
							} else if (res.cancel) {
								//
							}
			...
			comfirmBasicConfig(){
				...
				if (value) {
					...
					transformFormat.savePz(temp, uni, 'under_'+this.project.name, plus, true);
				}
				uni.navigateTo({
			...
			confirmEditMainChannel(i, index) {
				...
				// if (si !== -1) {
				// 	this.selectedMainChannels[si] = i;
				// }
				i.isShow = false;
			},
			bindChannel(i) {
